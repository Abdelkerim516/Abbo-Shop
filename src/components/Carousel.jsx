        ...
        <Slider {...settings}>
                {
                    data?.slice(0,7)?.map((item, index) => {
                        return <div key={index} className='bg-gradient-to-r from-[#463ab7] via-[#7a71d8] to-[#c0c0e7] -z-10'>
                            <div className='flex flex-col md:flex-row gap-10 justify-center h-[600px] my-20 md:my-0 items-center px-4'>
                                <div className='md:space-y-6 space-y-3'>
                                    <h3 className='text-amber-300 font-semibold font-sans text-sm'>Alimentez votre monde avec le meilleur de l'électronique</h3>
                                    ...
                                    <button className='bg-gradient-to-r from-red-500  text-white px-3 py-2 rounded-md cursor-pointer mt-2'>Achetez maintenant</button>
                                </div>
                                <div>
                                    <img src={item.image} alt={item.title} className='rounded-full w-[550px] hover:scale-105 transition-all shadow-2xl '/>
                                </div>
                            </div>
                        </div>
                    })
